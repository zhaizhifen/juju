...
func validatePlacementForSpaces(st *state.State, spaceNames *[]string, placement []string) error {
	...
	for _, v := range placement {
		...
		// Get the spaces that correspond with the names.
		spaces := make(network.SpaceInfos, len(*spaceNames))
		for i, sn := range *spaceNames {
			sp, err := st.Space(sn)
			if err != nil {
				return errors.Trace(err)
			}
			spaces[i] = sp.NetworkSpace()
		}

		for _, space := range spaces {
			inSpace := false
			for _, addr := range m.Addresses() {
				if addr.SpaceID == space.ID {
					inSpace = true
					break
				}
			}
			if !inSpace {
				return fmt.Errorf("machine %q has no addresses in space %q", p.Directive, space.Name)
			}
		}
	}
	return nil
