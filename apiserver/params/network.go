...
// From ProviderAddress returns an Address for the input ProviderAddress.
...
// ToProviderAddresses transforms multiple Addresses into a
// ProviderAddresses collection.
func ToMachineHostPorts(hps []HostPort) []network.MachineHostPort {
	mHps := make([]network.MachineHostPort, len(hps))
	for i, hp := range hps {
...
// MachineHostPort transforms the HostPort to a ProviderHostPort.
...
func ToProviderHostPorts(hps []HostPort) []network.ProviderHostPort {
	pHps := make([]network.ProviderHostPort, len(hps))
	for i, hp := range hps {
